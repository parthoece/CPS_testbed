...
class HMI1(HMI):
    ...
    def _display(self):
        try:
            self.__show_table()
            logger.info("Displayed the current table with latency {}ms".format(self._latency / 1000))
        except Exception as e:
            logger.error("Error while displaying table: %s", str(e), exc_info=True)

    def _operate(self):
        try:
            self.__update_massages()
            logger.info("Updated messages for all tags successfully.")
        except Exception as e:
            logger.error("Error while updating messages: %s", str(e), exc_info=True)
    ...
    def __get_formatted_value(self, tag):
        ...
        try:
            ...
            logger.warning("Error receiving value for tag %s: %s", tag, str(e))
...
if __name__ == '__main__':
    hmi1 = HMI1()
    logger.info("Starting HMI1 system.")
    hmi1.start()
